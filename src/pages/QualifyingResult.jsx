...
export function QualifyingScreen({ pendingQualifying, accent, category, onContinue }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs uppercase tracking-[0.2em] mb-1" style={{ color: COLORS.muted }}>Clasificación</div>
      ...
      <div className="text-xs mb-4" style={{ color: isWet ? COLORS.ice : COLORS.muted }}>
        {isWet ? "🌧️ Clasificación en mojado" : "☀️ Clasificación en seco"}
      </div>
        ...
        <div className="mb-4 space-y-2">
          {injured.map((r) => (
            <div key={r.id} className="rounded-md px-3 py-2 text-sm flex items-center gap-2"
              ...
              <span>
                <strong>{r.name}</strong> ({r.teamName}) se cae en clasificación — {r.injuryResult.name.toLowerCase()} (lesión {r.injuryResult.severityLabel}). No podrá disputar la carrera.
              </span>
            </div>
          ))}
        </div>
